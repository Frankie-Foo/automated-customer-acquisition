...
同时新增 Repository 写入接口；`028_pdca_workflow_guards.sql` 防止重复开放待办和重复邮件草稿映射。

### Phase 2：线索入口和去重（核心入口已完成）
...
当前已接入人工新建、CSV 导入和后台公司名单获客。官网询盘、Odoo 和通用 Webhook 保留为下一批入口。

### Phase 3：分配、评分和任务（已完成第一版）
...
销售端“今日待办”现在直接读取 `followup_tasks`，只展示当前用户名下任务。系统会按客户状态自动生成补资料、写草稿、审核、发送、二次触达、回复处理或退信处理任务。

### Phase 4：AI 触达草稿（邮件闭环已完成）
...
邮件草稿、审核、真实发送、打开、回复和退信已接入统一任务状态机；WhatsApp、LinkedIn 私信等渠道尚未开放自动发送。

### Phase 5：PDCA 经营闭环（指标底座已完成，管理视图持续补充）
