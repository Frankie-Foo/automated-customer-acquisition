const o={status:"",filter:"",search:"",user:null,usage:null,linkedinTaskId:null},N=document.querySelector("#notice"),de=document.querySelector("#metrics"),D=document.querySelector("#ops-report-content"),ce=document.querySelector("#admin-console"),W=document.querySelector("#admin-summary"),_=document.querySelector("#admin-users-table"),b=document.querySelector("#admin-senders-table"),le=document.querySelector("#followup-grid"),ue=document.querySelector("#lifecycle-grid"),me=document.querySelector("#workspace-empty"),X=document.querySelector("#workspace-content"),pe=document.querySelector("#workspace-profile"),R=document.querySelector("#activity-list"),Y=document.querySelector("#stage-analysis"),x=document.querySelector("#contacts-body"),V=document.querySelector("#readiness"),E=document.querySelector("#ready-pill"),y=document.querySelector("#login-screen"),$=document.querySelector("#login-form"),H=document.querySelector("#login-error"),S=document.querySelector("#password-change-form"),O=document.querySelector("#password-change-error"),z=document.querySelector("#account-name"),G=document.querySelector("#quota-status"),ye=document.querySelector("#logout-button"),C=document.querySelector("#linkedin-search-output");function i(e,t=""){N.textContent=e,N.className=`notice ${t}`.trim(),N.scrollIntoView({behavior:"smooth",block:"nearest"})}function fe(){N.className="notice hidden"}async function d(e,t={}){const n=await fetch(e,{headers:{"Content-Type":"application/json"},...t}),a=await n.json();if(n.status===401&&e!=="/api/login")throw P(),window.dispatchEvent(new CustomEvent("salesbot:unauthorized")),new Error("请先登录");if(!a.ok)throw new Error(a.error||"请求失败");return a.data}async function he(){if(!window.SALESBOT_REACT_AUTH)try{const e=await d("/api/me");if(o.user=e.user,o.usage=e.usage,g(),o.user.must_change_password){Z();return}A(),await l()}catch{P()}}function P(){if(window.SALESBOT_REACT_AUTH){y==null||y.classList.remove("hidden");return}y.classList.remove("hidden"),$.classList.remove("hidden"),S.classList.add("hidden")}function A(){if(window.SALESBOT_REACT_AUTH){y==null||y.classList.add("hidden");return}y.classList.add("hidden"),H.textContent="",O.textContent=""}function Z(){if(window.SALESBOT_REACT_AUTH){y==null||y.classList.remove("hidden");return}y.classList.remove("hidden"),$.classList.add("hidden"),S.classList.remove("hidden"),O.textContent=""}function g(){if(!o.user){z.textContent="未登录",G.textContent="今日配额 --",K();return}const e=o.usage||{};z.textContent=o.user.display_name||o.user.username,G.textContent=`获客 ${e.source_count||0}/${o.user.daily_source_limit} · 发信 ${e.send_count||0}/${o.user.daily_send_limit}`,ce.classList.toggle("hidden",o.user.role!=="admin"),K()}function q(e){e&&(o.usage=e,g())}function K(){window.SALESBOT_REACT_AUTH||window.dispatchEvent(new CustomEvent("salesbot:session",{detail:{user:o.user,usage:o.usage}}))}window.addEventListener("salesbot:session",e=>{var t,n;o.user=((t=e.detail)==null?void 0:t.user)||null,o.usage=((n=e.detail)==null?void 0:n.usage)||null,g(),o.user&&!o.user.must_change_password?A():o.user||P()});window.addEventListener("salesbot:refresh",l);window.addEventListener("salesbot:refresh-related",async()=>{try{const[e,t]=await Promise.all([d("/api/summary"),d("/api/lifecycle")]);M(e),I(t,window.latestContacts||[]),await L(),await k()}catch(e){i(e.message,"error")}});window.addEventListener("salesbot:usage",e=>{var t;return q((t=e.detail)==null?void 0:t.usage)});window.addEventListener("salesbot:contacts-updated",e=>{var n;const t=((n=e.detail)==null?void 0:n.contacts)||[];window.latestContacts=t,J(t),d("/api/lifecycle").then(a=>I(a,t)).catch(()=>{})});window.addEventListener("salesbot:open-contact",async e=>{var t;if(!window.SALESBOT_REACT_WORKSPACE)try{await v(Number((t=e.detail)==null?void 0:t.contactId)),i("客户详情已打开")}catch(n){i(n.message,"error")}});window.addEventListener("salesbot:notice",e=>{var t;(t=e.detail)!=null&&t.message&&i(e.detail.message,e.detail.type||"")});async function l(){try{const[e,t,n]=await Promise.all([d("/api/summary"),d(`/api/contacts?status=${encodeURIComponent(o.status)}&filter=${encodeURIComponent(o.filter)}&search=${encodeURIComponent(o.search)}&limit=100`),d("/api/lifecycle")]);fe(),M(e);const a=t.contacts||[];window.latestContacts=a,J(a),I(n,a),F(a),await L(),await k(),await re(),ve()}catch(e){M({total_contacts:0,sent_today:0,statuses:{},events_7d:{}}),J([]),I({stages:{}},[]),F([]),U({}),i(`数据库还不可用：${e.message}。先确认 .env/config.yaml，然后点“初始化/迁移数据库”。`,"error")}}async function L(){try{U(await d("/api/ops-report"))}catch{U({})}}window.addEventListener("salesbot:ops-refresh",L);async function k(){var e;if(!window.SALESBOT_REACT_ADMIN&&((e=o.user)==null?void 0:e.role)==="admin")try{const[t,n]=await Promise.all([d("/api/admin/users"),d("/api/admin/senders")]);we(t.users||[],n.senders||[]),be(t.users||[]),$e(n.senders||[])}catch(t){_.innerHTML=`<div class="empty-state">管理员数据加载失败：${s(t.message)}</div>`}}async function ve(){try{const e=await d("/api/readiness");E.textContent=e.ready?"Ready":"Action needed",E.className=e.ready?"ready":"missing",V.innerHTML=e.checks.map(t=>`
      <div class="check ${t.ok?"ok":"missing"}" title="${s(t.message||"")}">
        <span>${s(_e(t.name))}</span>
  ...
  `).join("")}function _e(e){return{database:"数据库连接",lead_source:"自动获客 API",enrichment:"邮箱富化 API",resend:"邮件发送 API",sender_email:"发件邮箱域名",dry_run:"真实发送开关",public_url:"公网访问地址",admin_password:"管理员密码",social_enrichment:"社媒富化 API",llm:"AI 文案模型",slack:"Slack 通知"}[e]||e}function J(e){const t=e.filter(c=>Number(c.opened_count||0)>0&&!["replied","bounced","unsubscribed"].includes(c.status)),n=e.filter(c=>c.status==="replied"||Number(c.replied_count||0)>0),a=e.filter(c=>c.status==="bounced"||Number(c.bounced_count||0)>0),r=[{title:"已打开未回复",count:t.length,hint:"建议今天人工跟进或准备下一封",tone:"hot",contacts:t},{title:"已回复",count:n.length,hint:"需要销售马上接手沟通",tone:"reply",contacts:n},{title:"退信需处理",count:a.length,hint:"检查邮箱质量或加入黑名单",tone:"risk",contacts:a}];le.innerHTML=r.map(ge).join("")}function ge(e){const t=e.contacts.slice(0,3).map(a=>`
    <li>
      <div>
        <strong>${s(T(a))}</strong>
  ...
  `}function be(e){if(!e.length){_.innerHTML='<div class="empty-state">暂无用户</div>';return}_.innerHTML=`
  ...
  `}const ee=[["lead","陌生线索"],["replied","已回复"],["conversation","初步沟通"],["meeting","约会/会议"],["business_plan","商业计划"],["store_visit","到店参观"],["trial_order","试订单"],["agency_agreement","代理协议"],["hq_visit","总部拜访"],["signed","成功签约"],["maintenance","持续维护"],["waiting_pool","等待池"],["abandoned","已放弃"]];function I(e,t){const n=e.stages||{};ue.innerHTML=ee.map(([a,r])=>{const c=n[a]||0,m=t.filter(f=>f.lifecycle_stage===a).slice(0,2).map(f=>`<span>${s(T(f))}</span>`).join("");return`
      <article class="lifecycle-card ${a}">
        <strong>${s(r)}</strong>
        <b>${c}</b>
        <div>${m||"<span>暂无客户</span>"}</div>
      </article>
    `}).join("")}function Se(e){return e.status==="replied"||Number(e.replied_count||0)>0?"已回复":e.status==="bounced"||Number(e.bounced_count||0)>0?"退信":Number(e.opened_count||0)>0?`打开 ${e.opened_count} 次`:ie(e.last_event_type||e.status)}function F(e){if(!window.SALESBOT_REACT_CONTACTS){if(!e.length){x.innerHTML=`
      <tr>
        <td colspan="13">
  ...
  `,await w(o.linkedinTaskId)}async function w(e){if(!e)return;const t=document.querySelector("#linkedin-result-panel");if(!t)return;const a=(await d(`/api/search-results?task_id=${encodeURIComponent(e)}`)).results||[];t.innerHTML=je(a)}function je(e){return e.length?`
  ...
  `:'<div class="empty-state compact"><strong>该任务暂无结果</strong><p>如果 Google CSE 没有返回内容，可以放宽职位或地区关键词。</p></div>'}function Re(e){return!Array.isArray(e)||!e.length?'<span class="muted">暂无邮箱候选</span>':e.slice(0,4).map(t=>`
    <span class="candidate-chip ${s(t.category||"")}">
      ${s(t.email||"")}
      <small>${Number(t.confidence||0)}%</small>
    </span>
  `).join("")}function xe(e){const t=e.criteria||{};return[t.role||t.title,t.industry,t.location,t.company_keyword].filter(Boolean).join(" / ")||"公开搜索"}function He(e){return{candidate:"候选",low_score:"低分跳过",promoted:"已入库",duplicate:"重复",failed:"失败"}[e]||e||"未知"}function Me(e,t){const n=Ve(t);if(e==="next")return{contact_id:t,lifecycle_stage:ze(n==null?void 0:n.lifecycle_stage),disposition:"active",notes:"dashboard: move to next lifecycle stage"};if(e==="wait"){const a=new Date(Date.now()+6048e5).toISOString();return{contact_id:t,lifecycle_stage:(n==null?void 0:n.lifecycle_stage)||"waiting_pool",disposition:"waiting",next_action_at:a,notes:"dashboard: move to waiting follow-up pool"}}return{contact_id:t,lifecycle_stage:"abandoned",disposition:"abandoned",lost_reason:"dashboard: manually abandoned",notes:"dashboard: abandon customer lifecycle"}}async function v(e){const t=await d(`/api/contact-detail?contact_id=${encodeURIComponent(e)}`);if(!t.contact)throw new Error("客户不存在");window.selectedContactId=e,window.selectedContactDetail=t,me.classList.add("hidden"),X.classList.remove("hidden"),Je(t.contact),De(t.activities||[]),Y.innerHTML="",document.querySelector("#activity-stage").value=t.contact.lifecycle_stage||"lead",document.querySelector("#email-mode").value="ai",document.querySelector("#email-subject").value=`Quick question about ${t.contact.company_name||"your business"}`,document.querySelector("#email-body").value="",document.querySelector("#activity-content").value="",document.querySelector("#customer-workspace").scrollIntoView({behavior:"smooth",block:"start"})}function Je(e){const t=e.profile_insights||{};pe.innerHTML=`
    <div>
      <strong>${s(T(e))}</strong>
  ...
  `:`
    ...
    `}function Be(e){return{personal_work:"个人工作邮箱",personal_free:"个人邮箱",company_generic:"公司通用邮箱"}[e]||"未分类"}function De(e){if(!e.length){R.innerHTML='<div class="empty-activity">还没有阶段记录。</div>';return}R.innerHTML=e.map(t=>`
    <article class="activity-card">
      <header>
        <strong>${s(B(t.lifecycle_stage))} / ${s(We(t.activity_type))}</strong>
  ...
  `).join("")}function We(e){return{reply:"回复内容",research:"客户资料/背景调研",meeting_note:"会议纪要",business_plan:"商业计划",trial_order:"试订单",agreement_review:"代理协议风险",store_plan:"门店创建资料",note:"普通备注"}[e]||e}function oe(e){if(!e||!Object.keys(e).length)return"";const t=(n,a)=>a!=null&&a.length?`<div><b>${n}</b>${a.map(r=>`<span>${s(r)}</span>`).join("")}</div>`:"";return`
  ...
  `}function Ve(e){var t;return(t=window.latestContacts)==null?void 0:t.find(n=>Number(n.id)===Number(e))}function ze(e){const t=["lead","replied","conversation","meeting","business_plan","store_visit","trial_order","agency_agreement","hq_visit","signed","maintenance"],n=t.indexOf(e||"lead");return t[Math.min(n+1,t.length-1)]}he();
//# sourceMappingURL=legacy-controller-1xJYKWgg.js.map
