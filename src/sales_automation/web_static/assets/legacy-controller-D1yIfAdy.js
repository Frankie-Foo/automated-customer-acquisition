const o={status:"",filter:"",search:"",user:null,usage:null,linkedinTaskId:null},L=document.querySelector("#notice"),de=document.querySelector("#metrics"),J=document.querySelector("#ops-report-content"),ce=document.querySelector("#admin-console"),U=document.querySelector("#admin-summary"),v=document.querySelector("#admin-users-table"),_=document.querySelector("#admin-senders-table"),le=document.querySelector("#followup-grid"),ue=document.querySelector("#lifecycle-grid"),me=document.querySelector("#workspace-empty"),X=document.querySelector("#workspace-content"),pe=document.querySelector("#workspace-profile"),j=document.querySelector("#activity-list"),Y=document.querySelector("#stage-analysis"),x=document.querySelector("#contacts-body"),B=document.querySelector("#readiness"),q=document.querySelector("#ready-pill"),y=document.querySelector("#login-screen"),w=document.querySelector("#login-form"),H=document.querySelector("#login-error"),b=document.querySelector("#password-change-form"),N=document.querySelector("#password-change-error"),D=document.querySelector("#account-name"),V=document.querySelector("#quota-status"),ye=document.querySelector("#logout-button"),T=document.querySelector("#linkedin-search-output");function i(e,t=""){L.textContent=e,L.className=`notice ${t}`.trim(),L.scrollIntoView({behavior:"smooth",block:"nearest"})}function fe(){L.className="notice hidden"}async function c(e,t={}){const n=await fetch(e,{headers:{"Content-Type":"application/json"},...t}),a=await n.json();if(n.status===401&&e!=="/api/login")throw I(),window.dispatchEvent(new CustomEvent("salesbot:unauthorized")),new Error("请先登录");if(!a.ok)throw new Error(a.error||"请求失败");return a.data}async function he(){if(!window.SALESBOT_REACT_AUTH)try{const e=await c("/api/me");if(o.user=e.user,o.usage=e.usage,g(),o.user.must_change_password){Z();return}O(),await l()}catch{I()}}function I(){if(window.SALESBOT_REACT_AUTH){y==null||y.classList.remove("hidden");return}y.classList.remove("hidden"),w.classList.remove("hidden"),b.classList.add("hidden")}function O(){if(window.SALESBOT_REACT_AUTH){y==null||y.classList.add("hidden");return}y.classList.add("hidden"),H.textContent="",N.textContent=""}function Z(){if(window.SALESBOT_REACT_AUTH){y==null||y.classList.remove("hidden");return}y.classList.remove("hidden"),w.classList.add("hidden"),b.classList.remove("hidden"),N.textContent=""}function g(){if(!o.user){D.textContent="未登录",V.textContent="今日配额 --",W();return}const e=o.usage||{};D.textContent=o.user.display_name||o.user.username,V.textContent=`获客 ${e.source_count||0}/${o.user.daily_source_limit} · 发信 ${e.send_count||0}/${o.user.daily_send_limit}`,ce.classList.toggle("hidden",o.user.role!=="admin"),W()}function A(e){e&&(o.usage=e,g())}function W(){window.SALESBOT_REACT_AUTH||window.dispatchEvent(new CustomEvent("salesbot:session",{detail:{user:o.user,usage:o.usage}}))}window.addEventListener("salesbot:session",e=>{var t,n;o.user=((t=e.detail)==null?void 0:t.user)||null,o.usage=((n=e.detail)==null?void 0:n.usage)||null,g(),o.user&&!o.user.must_change_password?O():o.user||I()});window.addEventListener("salesbot:refresh",l);window.addEventListener("salesbot:notice",e=>{var t;(t=e.detail)!=null&&t.message&&i(e.detail.message,e.detail.type||"")});async function l(){try{const[e,t,n]=await Promise.all([c("/api/summary"),c(`/api/contacts?status=${encodeURIComponent(o.status)}&filter=${encodeURIComponent(o.filter)}&search=${encodeURIComponent(o.search)}&limit=100`),c("/api/lifecycle")]);fe(),z(e);const a=t.contacts||[];window.latestContacts=a,G(a),F(n,a),K(a),await C(),await E(),await re(),ve()}catch(e){z({total_contacts:0,sent_today:0,statuses:{},events_7d:{}}),G([]),F({stages:{}},[]),K([]),R({}),i(`数据库还不可用：${e.message}。先确认 .env/config.yaml，然后点“初始化/迁移数据库”。`,"error")}}async function C(){try{R(await c("/api/ops-report"))}catch{R({})}}window.addEventListener("salesbot:ops-refresh",C);async function E(){var e;if(!window.SALESBOT_REACT_ADMIN&&((e=o.user)==null?void 0:e.role)==="admin")try{const[t,n]=await Promise.all([c("/api/admin/users"),c("/api/admin/senders")]);we(t.users||[],n.senders||[]),be(t.users||[]),$e(n.senders||[])}catch(t){v.innerHTML=`<div class="empty-state">管理员数据加载失败：${s(t.message)}</div>`}}async function ve(){try{const e=await c("/api/readiness");q.textContent=e.ready?"Ready":"Action needed",q.className=e.ready?"ready":"missing",B.innerHTML=e.checks.map(t=>`
      <div class="check ${t.ok?"ok":"missing"}" title="${s(t.message||"")}">
        <span>${s(ge(t.name))}</span>
        <strong>${t.ok?"OK":t.required?"缺失":"可选"}</strong>
      </div>
    `).join("")}catch{q.textContent="Error",q.className="missing",B.innerHTML='<div class="check missing"><span>readiness</span><strong>失败</strong></div>'}}function z(e){var a,r;const t=e.events_7d||{},n=[["客户总数",e.total_contacts||0,"系统内全部客户"],["今日发送",e.sent_today||0,"当天真实/演练发送"],["待发送",((a=e.statuses)==null?void 0:a.queued)||0,"已入队等待触达"],["7天打开",t.opened||0,"最近 7 天打开事件"],["已回复",((r=e.statuses)==null?void 0:r.replied)||0,"需要销售跟进"]];de.innerHTML=n.map(([d,p,m])=>`
    <div class="metric">
      <span>${d}</span>
      <strong>${p}</strong>
      <small>${m}</small>
  ...
  `}function R(e){var m,f;const t=e.totals||{},n=e.events||{},a=(e.scope||"")==="team"||((m=o.user)==null?void 0:m.role)==="admin",r=(e.provider_stats||[]).slice(0,8).map(u=>`
    <tr>
      <td>${s(u.provider)}</td>
      <td>${u.calls||0}</td>
  ...
  `,a||(f=J.querySelectorAll(".ops-grid section")[1])==null||f.remove()}function h(e,t){return`<article><span>${s(e)}</span><strong>${Number(t||0)}</strong></article>`}function we(e,t){if(!U)return;const n=e.filter(m=>m.active).length,a=e.filter(m=>m.must_change_password).length,r=e.reduce((m,f)=>m+Number(f.source_count_today||0),0),d=e.reduce((m,f)=>m+Number(f.send_count_today||0),0),p=t.filter(m=>m.active).length;U.innerHTML=`
    <article>
      <span>销售账号</span>
  ...
  `}function $e(e){if(!e.length){_.innerHTML='<div class="empty-state">暂无发件账号。先在 config.yaml 的 sender_pool.accounts[] 配置。</div>';return}_.innerHTML=`
    <table class="mini-table admin-data-table">
  ...
  `}const ee=[["lead","陌生线索"],["replied","已回复"],["conversation","初步沟通"],["meeting","约会/会议"],["business_plan","商业计划"],["store_visit","到店参观"],["trial_order","试订单"],["agency_agreement","代理协议"],["hq_visit","总部拜访"],["signed","成功签约"],["maintenance","持续维护"],["waiting_pool","等待池"],["abandoned","已放弃"]];function F(e,t){const n=e.stages||{};ue.innerHTML=ee.map(([a,r])=>{const d=n[a]||0,m=t.filter(f=>f.lifecycle_stage===a).slice(0,2).map(f=>`<span>${s(S(f))}</span>`).join("");return`
      <article class="lifecycle-card ${a}">
        <strong>${s(r)}</strong>
        <b>${d}</b>
        <div>${m||"<span>暂无客户</span>"}</div>
      </article>
    `}).join("")}function Se(e){return e.status==="replied"||Number(e.replied_count||0)>0?"已回复":e.status==="bounced"||Number(e.bounced_count||0)>0?"退信":Number(e.opened_count||0)>0?`打开 ${e.opened_count} 次`:ie(e.last_event_type||e.status)}function K(e){if(!e.length){x.innerHTML=`
      <tr>
        <td colspan="13">
  ...
  `}function se(e){return{high:"高意向",medium:"中意向",low:"低意向",unknown:"待判断"}[e]||"待判断"}function Ce(e){return`
  ...
  `}function Ie(e){const t=[];if(Number(e.sent_count||0)>0&&t.push(["sent",`已发送 ${e.sent_count}`]),Number(e.opened_count||0)>0&&t.push(["opened",`已打开 ${e.opened_count}`]),Number(e.clicked_count||0)>0&&t.push(["clicked",`已点击 ${e.clicked_count}`]),Number(e.replied_count||0)>0&&t.push(["replied","已回复"]),Number(e.bounced_count||0)>0&&t.push(["bounced","已退信"]),Number(e.unsubscribed_count||0)>0&&t.push(["unsubscribed","已退订"]),!t.length)return'<span class="muted">暂无反馈</span>';const n=t.map(([r,d])=>`<span class="event-chip ${r}">${s(d)}</span>`).join(""),a=e.last_event_type?`<div class="muted">最近：${s(ie(e.last_event_type))} ${P(e.last_event_at)}</div>`:"";return`<div class="event-list">${n}${a}</div>`}function ie(e){return{sent:"已发送",opened:"已打开",clicked:"已点击",replied:"已回复",bounced:"已退信",unsubscribed:"已退订"}[e]||e}function P(e){return e?new Date(e).toLocaleString():""}function s(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function Ae(e){try{const t=String(e||"").trim();if(!t)return"";let a=new URL(t.includes("://")?t:`https://${t}`).hostname.toLowerCase();a.startsWith("www.")&&(a=a.slice(4));const r=a.split(".").filter(Boolean);return r.length>2&&(a=r.slice(-2).join(".")),a}catch{return String(e||"").trim().replace(/^https?:\/\//,"").replace(/^www\./,"").split("/")[0]}}async function Pe(e){i("正在执行，请稍等...");const t=await c(`/api/${e}`,{method:"POST",body:JSON.stringify({limit:100})});A(t.usage),i(`完成：${JSON.stringify(t)}`),await l()}document.querySelectorAll("[data-action]").forEach(e=>{e.addEventListener("click",async()=>{try{await Pe(e.dataset.action)}catch(t){i(t.message,"error")}})});document.querySelectorAll(".tab").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".tab").forEach(t=>t.classList.remove("active")),document.querySelectorAll(".tab-panel").forEach(t=>t.classList.remove("active")),e.classList.add("active"),document.querySelector(`#tab-${e.dataset.tab}`).classList.add("active")})});document.querySelector("#refresh-button").addEventListener("click",l);window.SALESBOT_REACT_AUTH||(w==null||w.addEventListener("submit",async e=>{e.preventDefault(),H.textContent="";try{const t=document.querySelector("#login-password").value,n=await c("/api/login",{method:"POST",body:JSON.stringify({username:document.querySelector("#login-username").value.trim(),password:t})});if(o.user=n.user,o.usage=n.usage,document.querySelector("#current-password").value=t,g(),o.user.must_change_password){Z();return}O(),await l()}catch(t){H.textContent=t.message}}),b==null||b.addEventListener("submit",async e=>{e.preventDefault(),N.textContent="";try{const t=document.querySelector("#current-password").value,n=document.querySelector("#new-password").value,a=document.querySelector("#confirm-password").value;if(n.length<12)throw new Error("新密码至少 12 位");if(n!==a)throw new Error("两次输入的新密码不一致");const r=await c("/api/change-password",{method:"POST",body:JSON.stringify({current_password:t,new_password:n})});o.user=r.user,document.querySelector("#login-password").value="",document.querySelector("#current-password").value="",document.querySelector("#new-password").value="",document.querySelector("#confirm-password").value="",g(),O(),i("密码已更新"),await l()}catch(t){N.textContent=t.message}}));ye.addEventListener("click",async()=>{await fetch("/api/logout"),o.user=null,o.usage=null,g(),window.dispatchEvent(new CustomEvent("salesbot:logout")),I()});document.querySelector("#export-button").addEventListener("click",()=>{const e=new URLSearchParams;o.status&&e.set("status",o.status),window.location.href=`/api/export.csv?${e.toString()}`});document.querySelector("#status-filter").addEventListener("change",e=>{o.status=e.target.value,l()});document.querySelector("#contact-filter").addEventListener("change",e=>{o.filter=e.target.value,l()});document.querySelector("#search-input").addEventListener("input",e=>{o.search=e.target.value,window.clearTimeout(window.searchTimer),window.searchTimer=window.setTimeout(l,250)});var Q;window.SALESBOT_REACT_ADMIN||((Q=document.querySelector("#admin-create-user"))==null||Q.addEventListener("click",async()=>{try{const e={username:document.querySelector("#admin-new-username").value.trim(),display_name:document.querySelector("#admin-new-display").value.trim(),password:document.querySelector("#admin-new-password").value,role:"sales",daily_source_limit:Number(document.querySelector("#admin-new-source-limit").value||100),daily_send_limit:Number(document.querySelector("#admin-new-send-limit").value||100)};if(!e.username||!e.password)throw new Error("账号和密码必填");await c("/api/admin/users",{method:"POST",body:JSON.stringify(e)}),i("销售账号已创建"),document.querySelector("#admin-new-password").value="",await E(),await C()}catch(e){i(e.message,"error")}}),v==null||v.addEventListener("click",async e=>{const t=e.target.closest("[data-admin-user-save]"),n=e.target.closest("[data-admin-user-toggle]"),a=e.target.closest("[data-admin-user-reset]");if(!(!t&&!n&&!a))try{const r=Number((t||n||a).dataset.adminUserSave||(t||n||a).dataset.adminUserToggle||(t||n||a).dataset.adminUserReset),d={user_id:r};if(t&&(d.daily_source_limit=Number(document.querySelector(`[data-user-source="${r}"]`).value||100),d.daily_send_limit=Number(document.querySelector(`[data-user-send="${r}"]`).value||100)),n&&(d.active=n.dataset.active==="true"),a){const p=window.prompt("输入新密码，至少 8 位");if(!p)return;if(p.length<8)throw new Error("密码至少 8 位");d.password=p}await c("/api/admin/user",{method:"POST",body:JSON.stringify(d)}),i("用户已更新"),await E(),await C()}catch(r){i(r.message,"error")}}),_==null||_.addEventListener("click",async e=>{const t=e.target.closest("[data-admin-sender-save]"),n=e.target.closest("[data-admin-sender-toggle]");if(!(!t&&!n))try{const a=Number((t||n).dataset.adminSenderSave||(t||n).dataset.adminSenderToggle),r={sender_id:a};t&&(r.daily_limit=Number(document.querySelector(`[data-sender-limit="${a}"]`).value||100),r.warmup_stage=document.querySelector(`[data-sender-warmup="${a}"]`).value),n&&(r.active=n.dataset.active==="true"),await c("/api/admin/sender",{method:"POST",body:JSON.stringify(r)}),i("发件账号已更新"),await E()}catch(a){i(a.message,"error")}}));document.querySelector("#mark-button").addEventListener("click",async()=>{try{const e=document.querySelector("#mark-id").value,t=document.querySelector("#mark-status").value;await c("/api/mark",{method:"POST",body:JSON.stringify({contact_id:Number(e),status:t})}),i("状态已更新"),await l()}catch(e){i(e.message,"error")}});document.querySelector("#blacklist-button").addEventListener("click",async()=>{try{await c("/api/blacklist",{method:"POST",body:JSON.stringify({email:document.querySelector("#blacklist-email").value||null,domain:document.querySelector("#blacklist-domain").value||null,reason:"dashboard"})}),i("黑名单已更新"),await l()}catch(e){i(e.message,"error")}});document.querySelector("#add-lead-button").addEventListener("click",async()=>{try{const e={linkedin_url:document.querySelector("#lead-linkedin").value,first_name:document.querySelector("#lead-first").value||null,last_name:document.querySelector("#lead-last").value||null,email:document.querySelector("#lead-email").value||null,email_status:document.querySelector("#lead-email").value?"valid":"unknown",status:document.querySelector("#lead-email").value?"enriched":"new",job_title:document.querySelector("#lead-title").value||null,company_name:document.querySelector("#lead-company").value||null,company_domain:document.querySelector("#lead-domain").value||null,source:"manual_dashboard"};if(!e.linkedin_url)throw new Error("LinkedIn URL 必填");const t=await c("/api/contacts",{method:"POST",body:JSON.stringify(e)});i(`新增完成：${JSON.stringify(t)}`),await l()}catch(e){i(e.message,"error")}});document.querySelector("#csv-import-button").addEventListener("click",async()=>{try{const e=document.querySelector("#csv-file").files[0];if(!e)throw new Error("请选择 CSV 文件");const t=await e.text(),n=await c("/api/import/csv",{method:"POST",body:JSON.stringify({csv:t,source:`csv:${e.name}`})});i(`CSV 导入完成：解析 ${n.parsed} 条，新增 ${n.inserted} 条，重复 ${n.skipped} 条`),await l()}catch(e){i(e.message,"error")}});document.querySelector("#source-button").addEventListener("click",async()=>{try{const e={company_website:Ae(document.querySelector("#source-company").value),role:document.querySelector("#source-role").value,industry:document.querySelector("#source-industry").value,location:document.querySelector("#source-location").value,limit:Number(document.querySelector("#source-limit").value||25)};if(!e.role)throw new Error("Role 必填");i("正在调用 Prospeo 自动获客，Limit 越大等待越久...");const t=await c("/api/source",{method:"POST",body:JSON.stringify(e)});A(t.usage);const n=t.result||[0,0];i(`自动获客完成：新增 ${n[0]} 条，重复 ${n[1]} 条`),await l()}catch(e){i(e.message,"error")}});document.querySelector("#linkedin-search-button").addEventListener("click",async()=>{try{const e={role:document.querySelector("#linkedin-role").value.trim(),industry:document.querySelector("#linkedin-industry").value.trim(),location:document.querySelector("#linkedin-location").value.trim(),company_keyword:document.querySelector("#linkedin-company").value.trim(),limit:Number(document.querySelector("#linkedin-limit").value||10),auto_domain_lookup:document.querySelector("#linkedin-auto-domain").checked,auto_generate_email_candidates:document.querySelector("#linkedin-auto-candidates").checked,high_confidence_verify:document.querySelector("#linkedin-high-verify").checked};if(!e.role&&!e.industry&&!e.company_keyword)throw new Error("至少填写职位、行业或公司关键词");i("正在通过 Google 公开索引搜索 LinkedIn 个人主页...");const t=await c("/api/source/linkedin-public-search",{method:"POST",body:JSON.stringify(e)});A(t.usage),o.linkedinTaskId=t.result.task_id,i(`LinkedIn 公网搜索完成：解析 ${t.result.results} 条，入库 ${t.result.promoted} 条，跳过 ${t.result.skipped} 条`),await l(),await $(o.linkedinTaskId)}catch(e){i(e.message,"error")}});document.querySelector("#linkedin-refresh-button").addEventListener("click",async()=>{try{await re(),o.linkedinTaskId&&await $(o.linkedinTaskId),i("LinkedIn 搜索结果已刷新")}catch(e){i(e.message,"error")}});T.addEventListener("click",async e=>{const t=e.target.closest("[data-search-task]"),n=e.target.closest("[data-promote-result]");try{if(t){o.linkedinTaskId=Number(t.dataset.searchTask),await $(o.linkedinTaskId);return}if(n){const a=Number(n.dataset.promoteResult),r=await c("/api/search-results/promote",{method:"POST",body:JSON.stringify({result_id:a})});i(r.contact_id?`已入库联系人 #${r.contact_id}`:"已处理，可能是重复客户"),await l(),o.linkedinTaskId&&await $(o.linkedinTaskId)}}catch(a){i(a.message,"error")}});x.addEventListener("click",async e=>{const t=e.target.closest("[data-life-action]");if(!t)return;const n=Number(t.dataset.id),a=t.dataset.lifeAction;try{if(a==="enrich-email"){i("正在富化当前客户邮箱...");const d=(await c("/api/enrich-one",{method:"POST",body:JSON.stringify({contact_id:n})})).fields||{};d.email_status==="valid"?i(`已找到邮箱：${d.email}`):i("没有找到已验证邮箱，稍后可以换数据源或补充更多客户信息再试","error")}else if(a==="enrich-social"){i("正在富化当前客户社媒...");const r=await c("/api/social-enrich-one",{method:"POST",body:JSON.stringify({contact_id:n})});i(r.ok?"社媒资料已更新":"没有找到可用社媒主页",r.ok?"":"error")}else if(a==="queue-one"){i("正在把当前客户加入发送队列...");const r=await c("/api/queue-one",{method:"POST",body:JSON.stringify({contact_id:n})});i(r.queued?"已加入发送队列":"未能入队：需要先有有效邮箱，且客户不能在黑名单里",r.queued?"":"error")}else if(a==="send-one"){i("正在发送当前客户的下一封邮件...");const r=await c("/api/send-one",{method:"POST",body:JSON.stringify({contact_id:n})});A(r.usage),i(r.sent?"邮件已发送":"未发送：需要先入队、满足发送间隔，并且未超过每日发送上限",r.sent?"":"error")}else if(a==="profile"){i("正在生成客户画像...");const d=(await c("/api/profile-agent",{method:"POST",body:JSON.stringify({contact_id:n})})).insights||{};i(`画像已更新：拟合度 ${d.icp_fit_score??"--"}，下一步：${d.next_action||d.summary||"已生成"}`)}else if(a==="detail")await k(n),i("客户详情已打开");else{const r=Me(a,n);await c("/api/lifecycle",{method:"POST",body:JSON.stringify(r)}),i("生命周期状态已更新")}await l()}catch(r){i(r.message,"error")}});document.querySelector("#save-activity-button").addEventListener("click",async()=>{try{if(!window.selectedContactId)throw new Error("请先选择客户");const e={contact_id:window.selectedContactId,lifecycle_stage:document.querySelector("#activity-stage").value,activity_type:document.querySelector("#activity-type").value,content:document.querySelector("#activity-content").value.trim(),created_by:"dashboard"};if(!e.content)throw new Error("请填写阶段记录");const t=await c("/api/lifecycle-activity",{method:"POST",body:JSON.stringify(e)});return i("阶段记录已保存"),await k(window.selectedContactId),await l(),t}catch(e){i(e.message,"error")}});document.querySelector("#stage-agent-button").addEventListener("click",async()=>{try{if(!window.selectedContactId)throw new Error("请先选择客户");const e={contact_id:window.selectedContactId,lifecycle_stage:document.querySelector("#activity-stage").value,activity_type:document.querySelector("#activity-type").value,content:document.querySelector("#activity-content").value.trim()};i("AI 正在分析当前阶段...");const t=await c("/api/stage-agent",{method:"POST",body:JSON.stringify(e)});Y.innerHTML=oe(t.analysis),i("AI 阶段分析已生成")}catch(e){i(e.message,"error")}});j.addEventListener("click",async e=>{const t=e.target.closest("[data-analyze-activity]");if(t)try{i("AI 正在重新分析记录..."),await c("/api/stage-agent",{method:"POST",body:JSON.stringify({contact_id:window.selectedContactId,activity_id:Number(t.dataset.analyzeActivity)})}),await k(window.selectedContactId),i("记录分析已更新")}catch(n){i(n.message,"error")}});X.addEventListener("click",async e=>{const t=e.target.closest("[data-adopt-email]");if(t)try{const n=Number(t.dataset.contactId),a=t.dataset.adoptEmail;await c("/api/email-candidates/adopt",{method:"POST",body:JSON.stringify({contact_id:n,email:a})}),i(`已采用候选邮箱：${a}`),await k(n),await l()}catch(n){i(n.message,"error")}});document.querySelector("#draft-email-button").addEventListener("click",async()=>{try{if(!window.selectedContactId)throw new Error("请先选择客户");i("正在生成邮件草稿...");const e=await c("/api/email-draft",{method:"POST",body:JSON.stringify({contact_id:window.selectedContactId,mode:document.querySelector("#email-mode").value,subject:document.querySelector("#email-subject").value,body:document.querySelector("#email-body").value})});document.querySelector("#email-subject").value=e.subject||"",document.querySelector("#email-body").value=e.body||"",i("邮件草稿已生成，请检查后再发送")}catch(e){i(e.message,"error")}});document.querySelector("#send-custom-email-button").addEventListener("click",async()=>{try{if(!window.selectedContactId)throw new Error("请先选择客户");const e=document.querySelector("#email-subject").value.trim(),t=document.querySelector("#email-body").value.trim();if(!e||!t)throw new Error("请先填写主题和正文");if(!window.confirm("确认发送给当前客户？dry_run=false 时会真实发出。"))return;i("正在发送邮件...");const n=await c("/api/send-custom",{method:"POST",body:JSON.stringify({contact_id:window.selectedContactId,mode:document.querySelector("#email-mode").value,subject:e,body:t})});i(`邮件已发送：第 ${n.step} 封`),await k(window.selectedContactId),await l()}catch(e){i(e.message,"error")}});async function re(){if(!T||!o.user)return;const t=(await c("/api/search-tasks")).tasks||[];if(!t.length){T.innerHTML=`
      <div class="empty-state compact">
        <strong>还没有 LinkedIn 公网搜索任务</strong>
        <p>填写上方条件后开始搜索，结果会先进入候选池和客户列表，不会自动发邮件。</p>
      </div>
    `;return}o.linkedinTaskId||(o.linkedinTaskId=t[0].id),T.innerHTML=`
  ...
  `,await $(o.linkedinTaskId)}async function $(e){if(!e)return;const t=document.querySelector("#linkedin-result-panel");if(!t)return;const a=(await c(`/api/search-results?task_id=${encodeURIComponent(e)}`)).results||[];t.innerHTML=je(a)}function je(e){return e.length?`
  ...
  `:`
    ...
    `}function Be(e){return{personal_work:"个人工作邮箱",personal_free:"个人邮箱",company_generic:"公司通用邮箱"}[e]||"未分类"}function De(e){if(!e.length){j.innerHTML='<div class="empty-activity">还没有阶段记录。</div>';return}j.innerHTML=e.map(t=>`
    <article class="activity-card">
      <header>
        <strong>${s(M(t.lifecycle_stage))} / ${s(Ve(t.activity_type))}</strong>
        <span>${P(t.created_at)}</span>
      </header>
      <p>${s(t.content)}</p>
      ${oe(t.ai_analysis)}
...
//# sourceMappingURL=legacy-controller-D1yIfAdy.js.map
